...
class ServerLauncher:
    ...
    def update_download_combo(self):
        """Rebuild download list hiding models already installed."""
        installed = set(self.available_models.keys())
        model_list = []
        for model_name, info in RECOMMENDED_MODELS.items():
            if model_name not in installed and info['recommended']:
                label = f"⭐ {model_name} ({info['size']}) - {info['category']}"
                model_list.append(label)
        for model_name, info in RECOMMENDED_MODELS.items():
            if model_name not in installed and not info['recommended']:
                label = f"{model_name} ({info['size']}) - {info['category']}"
                model_list.append(label)
        self.download_combo['values'] = model_list
        if model_list:
            phi_label = next((l for l in model_list if 'phi4-mini' in l), model_list[0])
            self.download_combo.set(phi_label)
        else:
            self.download_combo.set('All recommended models are installed')
    ...
    def download_selected_model(self):
        ...
        if model_name not in RECOMMENDED_MODELS:
            messagebox.showinfo("No Model", "Select a model from the list first.")
            return
